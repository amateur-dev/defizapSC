...
contract uniswapExchangeExecution is Ownable {
    ...
    // event
    event DaiHoldingsTransferred (uint, address);
    ...
    function ConvertETH2NEWDai() payable stopInEmergency public returns (uint) {
        //some basic checks
        ...
        uint DaiHoldings = NEW_DAI_TOKEN_ADDRESS.balanceOf(address(this));
        NEW_DAI_TOKEN_ADDRESS.transfer(msg.sender, DaiHoldings);
        emit DaiHoldingsTransferred(DaiHoldings, msg.sender);
        return DaiHoldings;
    }
