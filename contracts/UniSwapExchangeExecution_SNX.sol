...
contract uniswapExchangeExecution is Ownable {
    ...
    // event
    event SNXHoldingsTransferred (uint, address);
    ...
    function ConvertETH2SNX() payable stopInEmergency public returns (uint) {
        //some basic checks
        ...
        uint SNXHoldings = SNX_TOKEN_ADDRESS.balanceOf(address(this));
        SNX_TOKEN_ADDRESS.transfer(msg.sender, SNXHoldings);
        emit SNXHoldingsTransferred(SNXHoldings, msg.sender);
        return SNXHoldings;
    }
